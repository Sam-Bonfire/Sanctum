...
class KosherLocatorTest {

    @Test
    fun `builds search url from city`() {
        assertEquals(
            "https://nominatim.openstreetmap.org/search?q=kosher+restaurant+Brooklyn&format=json&limit=10",
            buildKosherSearchUrl("Brooklyn"),
        )
    }

    @Test
    fun `parses spots and skips invalid entries`() {
        val spots = parseKosherSpots(
    ...
    fun `returns empty for invalid json`() {
        assertTrue(parseKosherSpots("{}").isEmpty())
    }
}
