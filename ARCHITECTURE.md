...
- **Bring Your Own Cloud (BYOC)**: `ByocSyncManager` is the sync interface; check Koin bindings for the current platform implementation before demoing backup/restore. Bookmark tags persist in Room on mobile (`RoomBookmarkRepository`, schema v4); on web they remain an in-memory stub (`dataModule` in `core/di/Koin.kt`) because SQLite is unavailable there.
